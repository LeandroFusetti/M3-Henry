...
function problemA () {
  ...
  // promise version
  // ???
    //Promise.all ejecuta un sucess handler cuando todas las promesas se hayan resuelto
    //ejecuta un error handler cuando una de todas las promesas se haya rechazado
    Promise.all([
...
function problemB () {
  ...
  // promise version
  // ???
  const fileNames= [1,2,3,4,5,6,7,8].map(num=>`poem-two/stanza-0${num}.txt`)
